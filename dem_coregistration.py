...
#Personal libraries
import georaster.georaster as raster
...
class coregistration(QtCore.QRunnable):
    ...
    def __init__(self, master_dem_fp, slave_dem_fp, outfile, niter, plot, maskfile, shp, buffer, nodata1, nodata2, min_count, zmax, zmin, resmax, degree, grid, save, IS, doRamp):
        ...
        self.primary_dem = master_dem_fp
        self.secondary_dem = slave_dem_fp
        ...
        self.shp = shp
        self.buffer = buffer
        self.nodata1 = nodata1
        self.nodata2 = nodata2
    ...
    def run(self):
        ...
        self.coreg_with_master_dem(master_dem_fp=self.primary_dem, slave_dem_fp=self.secondary_dem, outfile=self.outfile, niter=self.niter,
                              plot=self.plot, maskfile=self.maskfile, shp=self.shp, buffer=self.buffer, nodata1=self.nodata1, nodata2=self.nodata2,
                              min_count=self.min_count, zmax=self.zmax, zmin=self.zmin, resmax=self.resmax, degree=self.degree, grid=self.grid,
                              save=self.save, IS=self.IS, doRamp=self.doRamp)
    ...
    def horizontal_shift(self, dh,slope,aspect,plot=False, min_count=30):
        """
    ...
    def deramping(self, diff,X,Y,d=1,plot=False):
      """
    ...
    def coreg_with_master_dem(self, master_dem_fp, slave_dem_fp, outfile, niter, plot, maskfile, shp, buffer, nodata1, nodata2, min_count, zmax, zmin, resmax, degree, grid, save, IS, doRamp):
    
        """
        Coregistration with the use of a master DEM
        """
        print('Beginning Co-Registration workflow...')
        ## Read DEMs metadata ##
        # master
        self.signal.updateSignal.emit("Loading Primary Raster...")
        self.signal.progressSignal.emit(1)
        master_dem = raster.SingleBandRaster(master_dem_fp,load_data=False)
        if nodata1!='none':
            nodata1 = float(nodata1)
        else:
            band=master_dem.ds.GetRasterBand(1)
            nodata1 = band.GetNoDataValue()
        print('Primary dem loaded')
        
        # slave
        self.signal.updateSignal.emit("Loading Secondary Raster...")
        self.signal.progressSignal.emit(1)
        slave_dem = raster.SingleBandRaster(slave_dem_fp,load_data=False)
        if nodata2!='none':
          nodata2 = float(nodata2)
        else:
        ...
        if master_dem.extent != slave_dem.extent:
          if grid=='master':
            print("Reproject secondary DEM")
            self.signal.updateSignal.emit("Reprojecting Secondary Raster")
            self.signal.progressSignal.emit(1)
            # Read extent of DEMs intersection in master projection
            extent = master_dem.intersection(slave_dem_fp)
    
            # Read master DEM in area of overlap
            master_dem = raster.SingleBandRaster(master_dem_fp,load_data=extent, latlon=False)
            master_dem.r = np.float32(master_dem.r)
            
            # Reproject slave DEM in same grid
            dem2coreg = slave_dem.reproject(master_dem.srs, master_dem.nx, master_dem.ny, master_dem.extent[0], master_dem.extent[3], master_dem.xres, master_dem.yres, dtype=6, nodata=nodata2, interp_type=gdal.GRA_Bilinear,progress=False).r
            ...
            self.signal.updateSignal.emit("Reprojection Primary Raster")
            self.signal.progressSignal.emit(1)
            # Read extent of DEMs intersection in slave projection
            extent = slave_dem.intersection(master_dem_fp)
            
            # Read slave DEM in area of overlap
            slave_dem = raster.SingleBandRaster(slave_dem_fp,load_data=extent, latlon=False)
          ...
          master_dem = raster.SingleBandRaster(master_dem_fp)
          master_dem.r = np.float32(master_dem.r)
          slave_dem = raster.SingleBandRaster(slave_dem_fp)
          dem2coreg = np.float32(slave_dem.r)
        ...
        ## mask points ##
        if maskfile!='none':
            self.signal.updateSignal.emit("Applying Mask")
            self.signal.progressSignal.emit(1)
            mask = raster.SingleBandRaster(maskfile)
        ...
        ## filter outliers ##
        if resmax!='none':
          master_dem.r[np.abs(master_dem.r-dem2coreg)>float(resmax)] = np.nan
          self.signal.updateSignal.emit("Filtering outliers....")
          self.signal.progressSignal.emit(1)
        ...
        if plot==True:
          ...
          #print("mean diff before: ")
          #print(np.mean(diff_before[np.isfinite(diff_before)]))
          #pl.imshow(diff_before,vmin=-maxval+median,vmax=maxval+median,cmap='RdYlBu')
          #cb=pl.colorbar()
          #cb.set_label('Elevation difference (m)')
          #pltname, ext = os.path.splitext(outfile)
          #pltname+="_beforePlot.png"
          #pl.savefig(pltname)
          #pl.show()
        ...
        self.signal.updateSignal.emit("Generating spline function...")
        self.signal.progressSignal.emit(1)
        ...
        self.signal.updateSignal.emit("Computing terrain statistics...")
        self.signal.progressSignal.emit(1)
        slope, aspect = self.grad2d(master_dem.r)
    
    
        ## Iterations to estimate DEMs shift
        print("Iteratively estimate DEMs shift")
        self.signal.updateSignal.emit("Calculating DEM shifts...")
        self.signal.progressSignal.emit(1)
    	...
    	# remove bias
            ...
            #compute offset
            east, north, c = self.horizontal_shift(dh,slope,aspect,plot,min_count)
            print("#%i - Offset in pixels : (%f,%f)" %(i+1,east,north))
            self.signal.updateSignal.emit("Shift %i: (%f,%f)" % (i+1, east, north))
            self.signal.progressSignal.emit(1)
            xoff+=east
            yoff+=north
        ...
        self.signal.updateSignal.emit("Final offset in pixels (east, north) : (%f, %f)" % (xoff, yoff))
        
        ### Deramping ###
        if degree>=0 and doRamp == True:
            self.signal.updateSignal.emit("Begin deramping workflow...")
            self.signal.progressSignal.emit(1)
            print("Deramping")
            diff = dem2coreg-master_dem.r
            ...
            if zmax!='none':
              diff[master_dem.r>int(zmax)] = np.nan
    
            # remove points below altitude threshold (e.g sea ice)
            if zmin!='none':
              diff[master_dem.r<int(zmin)] = np.nan
            ...
            self.signal.updateSignal.emit("Calculating and applying ramp...")
            self.signal.progressSignal.emit(1)
            
            ramp = self.deramping(diff,X,Y,d=degree,plot=plot)
            vshift = ramp(X,Y)
            dem2coreg-=vshift  #ramp(X,Y)
        else:
            print("deramping skipped")
        ...
        if save==True:
          ...
          self.signal.progressSignal.emit(1)
          self.signal.updateSignal.emit("Offset values saved to: %s" % fname)
        ...
        self.signal.updateSignal.emit("Median : %.2f, NMAD = %.2f" %(median,NMAD))
        ...
        if plot==True:
            ...
            diffBefore = np.mean(diff_before[np.isfinite(diff_before)])
            diffAfter = np.mean(diff_after[np.isfinite(diff_after)])
            print("Mean diff before:")
            print(np.mean(diff_before[np.isfinite(diff_before)]))
            print("Mean diff after:")
            print(np.mean(diff_after[np.isfinite(diff_after)]))
            f.write("\nElevation Difference pre-coregistration: %f\n" % diffBefore)
            f.write("Elevation Difference post-coregistration: %f" % diffAfter)
            pl.figure()
            pl.hist(diff_after[np.isfinite(diff_after)],bins=np.linspace(-maxval,maxval,50))
            pl.xlabel('DEM difference (m) - After')
            pltname, ext = os.path.splitext(outfile)
            pltname+= '_afterPlot.png'
            pl.savefig(pltname)
            
            pl.figure()
            pl.hist(diff_before[np.isfinite(diff_before)], bins=np.linspace(-maxval, maxval, 50))
            pl.xlabel('DEM difference (m) - Before')
            pltname, ext = os.path.splitext(outfile)
            pltname+= '_beforePlot.png'
            pl.savefig(pltname)
            
            print("Vertical difference plot saved.")
        ...
        raster.simple_write_geotiff(outfile, dem2coreg, gt, wkt=master_dem.srs.ExportToWkt(),dtype=gdal.GDT_Float32,nodata_value=nodata2)
        self.signal.updateSignal.emit("Done. Raster written to %s" % outfile)
        self.signal.progressSignal.emit(1)
        self.signal.finishedSignal.emit(1)
        
    def read_icesat_elev(self, is_files,RoI):
        """
    ...
    def coreg_with_IceSAT(self, args):
        """
        ...
        all_lons, all_lats, all_elev = self.read_icesat_elev(is_files,RoI)
        ...
        for i in range(args.niter):
            ...
            east, north, c = self.horizontal_shift(dh,slope_at_IS,aspect_at_IS,plot=args.plot, min_count=args.min_count)
            print("#%i - Offset in pixels : (%f,%f)" %(i+1,east,north))
            xoff+=east
            yoff+=north
    ...
    #    dh[np.isnan(slope)] = np.nan
        ...
        # estimate a ramp and remove it
        ramp = self.deramping(dh,xx,yy,d=args.degree,plot=False)
